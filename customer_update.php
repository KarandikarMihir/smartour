...
<div id="content" style="background-color:#eff4ff;height:380px;width:100%;display:inline-block;padding-left:70px">
    <div style="padding: 10px 10px 10px 0;">
        <div class="input-control text" style="width:36%;">
            <input type="text" name="searchparam" class="with-helper" onKeyUp="loadResults('resultset','customerUpdateResults','request.php',this.value)" placeholder="Serial number or name of the customer" style="font-family: 'Segoe UI Light', 'Open Sans', Verdana, Arial, Helvetica, sans-serif;" maxlength="40" required spellcheck="false" autocapitalize="off" autocorrect="off" autocomplete="off" /><button class="btn-search"></button>
        </div>
    </div>
                ...
                else
		{
                    ...
                    while($row = mysql_fetch_array($result))
                    {
                        ...
                        if(strlen($row['address'])>40)
                            echo '<td title="' . $row['address'] . '">' . mb_substr($row['address'], 0, 40) . '...</td>';
                        else
                            echo '<td>' . $row['address'] . '</td>';
                        echo '<td>' . $row['mobile'] . '</td>';
                        echo '<td><a href="customer_update_form.php?KEY=' . $row['srno'] . '" onclick="if(confirm(\'Click OK to confirm action\')) return true; else return false;">Select</a></td>';
			echo '</tr>';
                    }
		}
