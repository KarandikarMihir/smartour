...
<div id="content" style="background-color:#eff4ff;height:380px;width:100%;display:inline-block;padding-left:70px">
	...
	<?php
        ...
        {
            $result = mysql_query("SELECT srno, name, hotelname, chkin, chkout FROM application WHERE srno=$SearchParam AND balance>0 ORDER BY srno Asc;") or die(mysql_error());
        }
        else
	{
            $result = mysql_query("SELECT srno, name, hotelname, chkin, chkout FROM application WHERE name LIKE '%$SearchParam%' AND balance>0 ORDER BY srno Asc;") or die(mysql_error());
        }
