...
<div id="content" style="background-color:#eff4ff;height:380px;width:100%;display:inline-block;padding-left:70px">
	...
	<?php
        ...
        {
            $result = mysql_query("SELECT srno, name, hotelname, chkin, chkout FROM application WHERE srno=$SearchParam ORDER BY srno Asc;");
        }
        else
	{
            $result = mysql_query("SELECT srno, name, hotelname, chkin, chkout FROM application WHERE name LIKE '%$SearchParam%' ORDER BY srno Asc;");
        }
        if(!mysql_num_rows($result))
	{
        ...
        else
        {
            ...
            while($row = mysql_fetch_array($result))
            {
                ...
                echo '<td><a href="invoice_view.php?KEY=' . $row['srno'] . '" target="_blank">Print Preview</a></td>';
                echo '</tr>';
            }
	}
